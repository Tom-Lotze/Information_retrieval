...
for num_topics in vector_n_topics:
    ...
    # load model
    search_engine = lsa_lda.Search(model=model, model_type="lsi_tfidf", num_topics=num_topics)
    # collect results
    overall_ser = {}
    for qid in tqdm(qrels): 
        ...
        overall_ser[np.int64(qid)] = dict([(idx2key[idx], score) for idx, score in results])
