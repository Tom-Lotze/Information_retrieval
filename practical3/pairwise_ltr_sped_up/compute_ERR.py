import pairwise_ltr_sped_up as PAIR
import torch
import numpy as np
import sys
...
model = PAIR.FastRankNet()
model.load_state_dict(torch.load("./pairwise_ltr_sped_up/models/pairwise_128_7_0.005.pt"))
...
for qid in tqdm(np.arange(data.test.num_queries())):
    if evl.included(qid, data.test):
        ...
        ERR_query = PAIR.err(q_scores, q_labels)
