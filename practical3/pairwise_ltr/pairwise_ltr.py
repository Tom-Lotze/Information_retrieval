...
def loss_function(S, diffs):
    loss = (1/2) * (1 - S) * diffs + \
        torch.log(1 + torch.exp(-1 * diffs))
    return loss
    ...
    for epoch in range(n_epochs):
        ...
        with tqdm(total=len(train_dl)) as t:
            ...
            for step, (x_batch, y_batch) in enumerate(train_dl):
                ...
                if step % 50 == 0:
                    t.set_postfix_str(f'loss: {loss.mean().item()}')
